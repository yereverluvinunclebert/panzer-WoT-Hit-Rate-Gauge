...
var mainWindow, mainWindowwidthDefault, mainWindowheightDefault,
		...
		pin, lock, stopWatchState, mistake, tankHelpShow, facebookChat, hitrateTimer, secondHand,
		secondShadow, startButton, stopButton, helpButton, tickSwitch, letterBack, updatehitrate;
...
function setmenu() {
	mainWindow.onContextMenu = function () {
		...
		items.push(mItem);
	  	...
	  	mItem = new MenuItem();
	  	mItem.title = "Panzer Hit Rate Gauge Help";
	  	mItem.onSelect = function () {
		  	tankHelpShow();
	  	};
		items.push(mItem);
...
function settooltip() {
	if (preferences.tooltipPref.value === "enabled") {
		...
		switchFacesButton.tooltip = "Control undefined";
		dangerLamp.tooltip = "WoT hit rate colour";
	} else {
		background.tooltip = "";
		startButton.tooltip = "";
		stopButton.tooltip = "";
